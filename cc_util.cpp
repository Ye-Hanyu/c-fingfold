...
// 寻找文件
	...
	bool savexml(const string& file, string img_name, int width, int height, const vector<LabBBox>& objs){
		FILE* f = fopen(file.c_str(), "wb");
		if (!f) return false;
		//string imgpath = "D:/ssd/apple_data/" + img_name;
		string imgpath = "C:/Users/Administrator/Desktop/5156/" + img_name;
		fprintf(f, "<annotation>\n<folder>5156</folder>\n<filename>%s</filename>\n<path>%s</path>\n<size><width>%d</width><height>%d</height><depth>3</depth></size>\n", img_name.c_str(), imgpath.c_str(), width, height);
			...
			if ((*iter_ptr == '/' && iter_ptr != dir_ptr) || *iter_ptr == 0){
				...
				*iter_ptr = 0;
				if (!exists(dir_ptr)){
					if (!mkdir(dir_ptr)){
						if(!exists(dir_ptr)){
							//INFO("mkdir %s return false.", dir_ptr);
							return false;
						}
					}
				}
...
#define __GetTimeBlock						\
	...
	string timeNow(){
		...
		sprintf(time_string, "%04d_%02d_%02d_%02d_%02d_%02d", t.tm_year + 1900, t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec);
		return time_string;
	}
